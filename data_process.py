...
def iteror_raw_data(data_path, data_mark):
    """
       ...
       :param data_path：.mat文件所在路径 #i think this is txt file not mat?
    ...
    # 列出所有文件
    filenames = os.listdir(data_path) #return list of strings of filenames
    ...
    for single_txt in filenames:
        ...
        for key, _ in labels.items():
            if key in single_txt:
                label = labels[key] #label depends on the filename

        # 数据提取
        data = np.loadtxt(single_mat_path) #convert data in txt to ndarray
...
def data_augment(fs, win_tlen, overlap_rate, data_iteror):
    ...
    for iraw_data in data_iteror:
        single_raw_data = iraw_data[1].ravel() #flattened the data into 1D
        lab = iraw_data[0] #data label
        len_data = single_raw_data.shape[0] #data length
...
def preprocess(path, data_mark, fs, win_tlen,
    ...
    win_len = int(fs * win_tlen)
    data_iteror = iteror_raw_data(path, data_mark) #raw data paired with its label
    X, y = data_augment(fs, win_tlen, overlap_rate, data_iteror, **kargs) #window sliding result
    # print(len(y[y==0]))
